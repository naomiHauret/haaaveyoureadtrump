        ...
        <div class={cxs({ display: "flex", justifyContent: "flex-start", alignItems: "center" })}>
          ...
          >
            {state.currentBurger.length > 0 && (
              <Burger isSelectSliceView={true} isCustom={false} actions={actions} state={state} />
            )}
            {state.currentBurger.length === 0 && (
              <p>
          ...
          <div class={cxs({ marginLeft: `calc(${(3 / 12) * 100}% - 90px)`, width: `${(4 / 12) * 100}%` })}>
            ...
            >
              {state.currentBurger.length > 0 && (
                <Burger isSelectSliceView={false} isCustom={true} actions={actions} state={state} />
              )}
              {state.currentBurger.length === 0 && (
                <Burger isSelectSliceView={false} isCustom={false} actions={actions} state={state} />
              )}
            </div>
          </div>
        </div>
